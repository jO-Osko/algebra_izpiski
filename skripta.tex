...
\begin{remark}
Med drugim smo pokazali, da najmanjša podgrupa(podkolobar $\dots$), ki vsebuje dano množico res obstaja. Zadevo poa lahko dokažemo tudi hitreje, tako da vzamemo presek vseh podstruktur, ki to strukturo vsebujejo.
\end{remark}
...
\begin{example}
\begin{example_case}
$\mathbb{R}$ glejmo kot algebro nar $\mathbb{R}$. Direktni produkt števno kopij z operacijami po komponentah je algebra $\mathbb{R} \times \mathbb{R} \times \dots$.

Operacije po komponentah točno sovpadajo z operacijamo po komponentah za zaporedja. To je torej algebra realnih zaporedij.
\end{example_case}
\begin{example_case}
...
Množica $\mathcal{A}$ je dobro zadostuje \textbf{načelu dobre urejenosti} če vsaka neprazna podmnožica množice $\mathcal{A}$ vsebuje najmanjši element.
\end{definition}
...
\begin{statement}
$\mathbb{N}$ ustrezajo načelo dobre urejenosti.
\end{statement}
\begin{proof}\leavevmode\\
Z indukcijo na $n$:
$n=1$: $1 \notin \mathbb{N}$
...
Isto velja po indukciji tudi za $\mathbb{N} \cup \{0\}$, $\mathbb{N} \cup \{0, -1\}$, $\mathbb{N} \cup \{0, -1, -2\}$ $\dots$


Torej: Vsaka nerpazna navzdol omejena podmnožica $\mathbb{Z}$ vsebuje najmanjše število.
