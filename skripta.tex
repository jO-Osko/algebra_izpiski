...
Množenje vpelejemo po kosih in sicer: $1$ je enota za množenje, za druge pa velja:
\begin{equation}
...
\begin{remark}
Če si še enkrat pogledamo definicijo množenja polinomov (\ref{eq:def_polynomial_multiplication}) in pozabimo na pogoj, da so od nekje naprej vsi členi enaki $0$ potem govorimo o \textbf{kolobarju formalnih potenčnih vrst}, ki ga označimo z $$\mathcal{K}[[X]]^k$$ 
\end{remark}
...
2. $\exists m \in \mathcal{N}. \ \exists n \neq m \in \mathbb{N}. \ a^n = a^m$\\
...
Naj bo $a$ element grupe $\mathcal{G}$. Če bostaja tak $s \in \mathbb{N}$, da velja $a^s = 1$ ( $1$ je enota grupe), rečemo da ima $a$ \textbf{končen red}, najmanjšemu naravnemu številu s to lastnostjo pravimo \textbf{red elementa} $a$. Če pa takega elementa ni, potem pravimo, da ima $a$ \textbf{neskončen red}.
\end{definition}
...
\end{example_case}
\begin{example_case}
V $(\mathbb{Z}, +)$ imajo vsi elementi razem $0$ neskončen red.
\end{example_case}
\begin{example_case}
$\mathbb{Z}_4$, $1$ in $3$ imata red $4$, $2$ ima red $2$.
\end{example_case}
\begin{example_case}
$\mathcal{D}_4$(Simetrije pravokotnika, ki ni kvadrat) imajo vsi razen enote red $2$
\end{example_case}
\end{example}
...
Ker so si bazni vektorji neodvisni so vsi skaraji ničelni: $\lambda_i = 0 $.\\
