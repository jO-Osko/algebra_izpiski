...
\end{example_case}
\begin{example_case}
$\mathbb{C}$ je vektorski prostor nad $\mathbb{R}$ ( za $+$ je Abelova grupa, množenje pa definiramo po komponentah, tako je nad $\mathbb{C}$ to $2$-dimenzionalen, nad $\mathbb{R}$ pa $1$-dimenzionalen)
\end{example_case}
\end{example}
...
\begin{remark}
Za $\lambda = \mu = 1$ je to navadna distributivnost. Torje je Algebra kolobar, ki je hkrati vekroski prostor, v katerem velja še:
...
\begin{example}
\begin{example_case}
$\mathbb{Q}^+$ je grupa za množenje. Velja: $<\mathbb{N}> = \mathbb{Q}^+$, ker $<2,3> = \{2^i 3^j | i,j \in \mathbb{Z}\}$
\end{example_case}
\end{example}
...
x = \lambda1 x_{11} x_{12} \dots x_{1n_1} + \dots + \lambda_r x_{r1} x_{rn_r}; \lambda_i \in \mathcal{F} \land x_i \in \mathcal{X} \cup \{1\}
\end{equation}
\end{definition}
