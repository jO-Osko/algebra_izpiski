...
Naj bosta $m,k \in \mathbb{Z}$, šečemo, da \textbf{$k$ deli $m$} (pišemo tudi $k|m$), če obstaja tak $q \in \mathbb{Z}$, da velja $m = qk$.
\end{definition}
...
Dokazali smo, da je delitelj, potrebno je še dokazati da je največji.\\
...
\begin{corollary}
Celi števili $m,n$ sta si tuji natanko tedaj, ko obstajata taki celi števili $x,y$, da zadostita enačbi:
$$1=mx + ny$$
\end{corollary}
...
\label{th:fundamental_theroem_of_arithmetics}Vsako naravno število $n \geq 2$ lahko napišemo kot produkt praštevil. Ta zapis je do vrstnega reda faktorjev enoličen.
\end{theorem}
...
$p_1 | q_1 * q_2 * \dots * q_s$ torej po lemi (\ref{lem:prime_divisibility}) deli enega natančno enega izmed faktorjev $q$. Brez škode za splošnost: $p_1|q_1 \underbrace{\implies}_{\text{ker sta praštevili}} p_1 = q_1$ Krajšamo s $p_1$ in nadaljujemo dokler ne pridemo do $1=1$.\\
...
$$(ab) - (a' b') = \underbrace{b(a-a') + a(b-b')}_{\text{sta su kongruentna}}$$
\end{proof}
...
Ker je relacija ekvivalentna lahko vpeljemo ekvivačne razrede $[a]$.
...
Potrebno je preveriti še dobro definiranost za operacije.

\begin{statement}
Če v množici $\mathbb{Z}_n$ vpeljemo sešetevanje:
\begin{equation}
